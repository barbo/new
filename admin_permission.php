	...
	else
	{
		...
		if($permissionDetails['name'] != $_POST['name']) {
			...
			if (permissionNameExists($permission)){
				$errors[] = langu("ACCOUNT_PERMISSIONNAME_IN_USE", array($permission));
			}
			elseif (minMaxRange(1, 50, $permission)){
				$errors[] = langu("ACCOUNT_PERMISSION_CHAR_LIMIT", array(1, 50));	
			}
			else {
				if (updatePermissionName($permissionId, $permission)){
					$successes[] = langu("PERMISSION_NAME_UPDATE", array($permission));
				}
				else {
					$errors[] = langu("SQL_ERROR");
				}
		...
		if(!empty($_POST['removePermission'])){
			...
			if ($deletion_count = removePermission($permissionId, $remove)) {
				$successes[] = langu("PERMISSION_REMOVE_USERS", array($deletion_count));
			}
			else {
				$errors[] = langu("SQL_ERROR");
			}
		...
		if(!empty($_POST['addPermission'])){
			...
			if ($addition_count = addPermission($permissionId, $add)) {
				$successes[] = langu("PERMISSION_ADD_USERS", array($addition_count));
			}
			else {
				$errors[] = langu("SQL_ERROR");
			}
		...
		if(!empty($_POST['removePage'])){
			...
			if ($deletion_count = removePage($remove, $permissionId)) {
				$successes[] = langu("PERMISSION_REMOVE_PAGES", array($deletion_count));
			}
			else {
				$errors[] = langu("SQL_ERROR");
			}
		...
		if(!empty($_POST['addPage'])){
			...
			if ($addition_count = addPage($add, $permissionId)) {
				$successes[] = langu("PERMISSION_ADD_PAGES", array($addition_count));
			}
			else {
				$errors[] = langu("SQL_ERROR");
			}
...
<div id='content'>
<h1>My Form</h1>
<h2>".$languas[] = langu('Admin_permission')."</h2>
<div id='left-nav'>";
...
<table class='admin'>
<tr><td>
<h3>".$languas[] = langu('Permission_Information')."</h3>
<div id='regbox'>
<p>
...
<td>
<h3>".$languas[] = langu('Permission_Access')."</h3>
<div id='regbox'>
<p>
".$languas[] = langu('Public_access')."";
